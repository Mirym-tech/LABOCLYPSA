...
<div class="flex flex-wrap items-start gap-2 mb-4">
    ...
    <div class="min-w-0 flex-1">
        <h1 class="text-lg sm:text-xl font-bold text-gray-800 leading-tight">Bacteriología — Orden <span class="text-blue-600 font-mono">{{ $oa->orden->numero_orden }}</span></h1>
        <p class="text-sm text-gray-500">{{ $paciente->nombre }}</p>
    </div>
    <a href="{{ route('pdf.bacteriologia', $oa) }}" target="_blank"
        class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 flex-shrink-0">
