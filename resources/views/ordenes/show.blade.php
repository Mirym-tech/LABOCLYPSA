...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Orden <span class="text-blue-600 font-mono">{{ $orden->numero_orden }}</span></h1>
    ...
    <div class="ml-auto flex gap-2">
        ...
        <form method="POST" action="{{ route('ordenes.validar', $orden) }}">
            @csrf
            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
                <i class="fas fa-check-double"></i> Validar Orden
            </button>
        </form>
        @endrole
        @endif
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    ...
    <div class="lg:col-span-2 bg-white rounded-xl shadow-sm p-5">
        ...
        <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
            ...
            <div><span class="text-gray-400">Seguro:</span> {{ $orden->paciente->seguro_medico ?? '—' }}</div>
    ...
    <table class="w-full text-sm">
        ...
        @foreach($orden->analisis as $oa)
            @php
                ...
                $colores = ['pendiente'=>'yellow','en_proceso'=>'blue','listo'=>'green'];
                $c = $colores[$oa->estado] ?? 'gray';
            @endphp
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3">
                    <span class="px-2 py-0.5 rounded-full text-xs bg-{{ $c }}-100 text-{{ $c }}-800">
                        {{ ucfirst($oa->estado) }}
                    </span>
                </td>
